      ...
      <b-col md="12" class="mt-4 mb-4">
        ...
        >
          <b-card-body>
            ...
            >
              <b-card-body>
                <table class="table table-striped table-hover">
                  ...
                  <tbody>
                    <tr
                      v-for="(subscription, index) in user_profile.user_subscriptions"
                      :key="index"
                    >
            ...
            <b-card
              v-if="user_profile.user_builds.length > 0"
              :title="'Builds by ' + user_profile.username"
              :sub-title="'Total Builds: ' + user_profile.user_builds.length"
            >
              <b-card-body>
                <div v-for="(build, index) in user_profile.user_builds" :key="index">
                  <div v-if="$store.getters.getUser.user_id === build.author_id">
                    <b-button 
                      variant="success" 
                      size="sm"
                      @click="$router.push('/' + build.game_id + '/builds/editBuild/' + build.build_id)"
                    >Update</b-button>
                    <b-button
                      variant="danger"
                      size="sm"
                      @click="deleteBuild(build.build_id, build.rating_id)"
                    >Delete</b-button>
                  </div>
                  <nuxt-link
                    class="custom-link"
                  ...
                  >
                    <b-card :title="build.build_description">
                      <b-row>
                        <b-col md="2">
            ...
            <b-card
              v-if="user_profile.forum_posts.length > 0"
              :title="'Forum Posts by ' + user_profile.username"
              :sub-title="'Total Posts: ' + user_profile.forum_posts.length"
            >
              <b-card-body>
                <div v-for="(forum_post, index) in user_profile.forum_posts" :key="index">
                  <div v-if="$store.getters.getUser.user_id === forum_post.author_id">
                    <b-button 
                      variant="success" 
                      size="sm"
                      @click="$router.push('/' + forum_post.game_id + '/forum/editForumPost/' + forum_post.post_id)"
                    >Update</b-button>
                    <b-button
                      variant="danger"
                      size="sm"
                      @click="deletePost(forum_post.post_id, forum_post.rating_id)"
                    >Delete</b-button>
                  </div>
                  <nuxt-link
                    class="custom-link"
                  ...
                  >
                    <b-card :title="forum_post.post_description">
                      <b-row>
                        ...
                        <b-col md="2">
                          <font-awesome-icon :icon="['far', 'thumbs-up']"/>
                          Likes: {{ forum_post.up_vote }}
                        </b-col>
                        <b-col md="2">
                          <font-awesome-icon :icon="['far', 'thumbs-down']"/>
                          Dislikes: {{ forum_post.down_vote }}
                        </b-col>
                        <b-col md="6">
                          <span
                            class="float-right"
            ...
            <div class="mt-3" v-if="isAdmin || isUsersProfile">
              <b-button class="mt-3 mr-1" variant="success" @click="$router.push('/profile/updateProfile/' + user_profile.user_id)">
                <font-awesome-icon :icon="['far', 'edit']"/>&nbsp;Update Account
              </b-button>
              <b-button class="mt-3" variant="danger" @click="deleteAccount()">
                <font-awesome-icon :icon="['far', 'times-circle']"/>&nbsp;Delete Account
              </b-button>
            </div>
          </b-card-body>
        </b-card>
...
export default {
  ...
  methods: {
    ...
    deleteAccount() {
      let confirm = confirm(
        "Are you sure you want to delete this account? All builds and forum posts accociated with this account will be deleted."
      );
      if (confirm) {
        this.deleteMethod(
          {
  ...
  computed: {
    ...
    isUsersProfile() {
      return this.$store.getters.getUser.user_id === this.user_profile.user_id;
    }
  }
};
</script>
